 ...
 * 登录日志状态枚举
 *
 * <p>对应 sys_login_log 表的 status 字段：0=失败 1=成功</p>
 *
 * @author platform
 */
public enum LoginLogStatusEnum {
     ...
     */
    public static LoginLogStatusEnum getByCode(Integer code) {
        if (Objects.isNull(code)) {
            return null;
        }
        for (LoginLogStatusEnum value : values()) {
    ...
    public static String getDescByCode(Integer code) {
        return Optional.ofNullable(getByCode(code))
                .map(LoginLogStatusEnum::getDesc)
                .orElse("");
    }
